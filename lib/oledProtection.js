...
// Extra margin, in pixels, added on top of the configured shift amount when
// overscanning background actors — rounding/timing slack so a step never
// exposes a sliver of empty stage at the trailing edge.
const OVERSCAN_SAFETY_PX = 2;
...
export class OledProtectionManager {
    ...
    _applyPixelShiftStep() {
        ...
        for (const actor of this._backgroundActors()) {
            this._ensureOverscan(actor, amount);
            actor.ease({
    ...
    /**
     * Background actors (the real desktop background, and our own live
     * wallpaper actor in most display modes) are sized to exactly cover
     * the stage with no slack, so nudging one by a few pixels for burn-in
     * protection would drag its edge inward and expose bare stage on the
     * side it moved away from. Scaling the actor up slightly around its
     * center — by just enough margin to cover the configured shift amount
     * — keeps every edge overscanned past the stage bounds no matter which
     * step of the shift pattern is active.
     */
    _ensureOverscan(actor, amount) {
        const width = actor.width;
        const height = actor.height;
        if (width <= 0 || height <= 0)
            return;

        const margin = amount + OVERSCAN_SAFETY_PX;
        actor.pivot_point = new Graphene.Point({x: 0.5, y: 0.5});
        actor.scale_x = 1 + (2 * margin) / width;
        actor.scale_y = 1 + (2 * margin) / height;
    }
