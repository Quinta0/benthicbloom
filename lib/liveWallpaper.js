...
export class LiveWallpaperManager {
    ...
    disable() {
        ...
        this._unpatchShellIntegration();
        this._stop();
        this._clearTransition();
    }

    /**
     * Two of gnome-shell's own animations paint a *second*, independent
     * background on top of ours, hiding the live wallpaper behind a frozen
     * static image: the workspace-switch swipe (which clones the desktop
     * picture per-workspace so it can slide, see WorkspaceBackground in
     * workspaceAnimation.js) and the lock screen (which blurs its own
     * separate copy of the desktop picture, see UnlockDialog in
     * unlockDialog.js). Neither exposes a public hook for this, so both are
     * patched at the prototype level — wrapped in try/catch per-patch since
     * this reaches into shell-private methods that could rename across
     * versions, and one patch failing shouldn't take down the other.
     */
    _patchShellIntegration() {
        const manager = this;

        try {
            const {WorkspaceBackground} = WorkspaceAnimation;
            this._patchMethod(WorkspaceBackground.prototype, '_createBackground', original => function (...args) {
                const result = original.call(this, ...args);
                // Only the freshly-added static background clone is hidden
                // (not the whole WorkspaceBackground group), so any real
                // desktop-icon windows it also hosts stay visible.
                const staticBackground = this.get_last_child();
                if (manager.isActive && staticBackground)
                    staticBackground.visible = false;
                return result;
            });
        } catch (e) {
            this._logger.debug(`Workspace-switch live wallpaper integration unavailable (${e.message ?? e})`);
        }

        try {
            const {UnlockDialog} = UnlockDialogModule;
            this._patchMethod(UnlockDialog.prototype, '_createBackground', original => function (monitorIndex) {
                const result = original.call(this, monitorIndex);
                manager._attachLockScreenActor(this._backgroundGroup.get_last_child());
                return result;
            });
        } catch (e) {
            this._logger.debug(`Lock screen live wallpaper integration unavailable (${e.message ?? e})`);
        }
    }

    _patchMethod(obj, name, wrap) {
        const original = obj[name];
        obj[name] = wrap(original);
        this._patchedMethods.push({obj, name, original});
    }

    _unpatchShellIntegration() {
        for (const {obj, name, original} of this._patchedMethods)
            obj[name] = original;
        this._patchedMethods = [];
        this._lockScreenActors = [];
    }
    ...
    /**
     * Adds an actor sharing our live frame's Content onto the lock screen's
     * already-blurred per-monitor background widget (native Shell.BlurEffect
     * radius 90, see UnlockDialog._createBackground) — reusing gnome-shell's
     * own blur rather than applying a second one. Content is a step behind
     * a frame: _updateFrame() re-points every tracked actor at `_image`
     * each time a new one is decoded, so this needs no per-frame decode of
     * its own and needs no update here if playback hasn't started yet
     * (nothing to show is just transparent, revealing the native static
     * background beneath).
     */
    _attachLockScreenActor(container) {
        if (!container || !this._available || !this._settings.get_boolean(SettingsKey.APPLY_TO_LOCK_SCREEN))
            return;
        ...
        this._lockScreenActors.push(actor);
        actor.connect('destroy', () => {
            this._lockScreenActors = this._lockScreenActors.filter(a => a !== actor);
        });
    }
    ...
    _start() {
        ...
        try {
            ...
            // Seeking here immediately would race the pipeline's preroll —
            // GStreamer can only seek once a state change has actually
            // completed, so an early seek is silently dropped and playback
            // sticks at native 1.0x. Instead, defer to the first ASYNC_DONE
            // bus message (handled in _onBusMessage), which fires once the
            // pipeline is actually ready to seek.
            const initialRate = this._settings.get_double(SettingsKey.LIVE_WALLPAPER_PLAYBACK_RATE);
            this._pendingInitialRateSeek = initialRate !== 1.0;
            this._logger.debug(
                `Live wallpaper: configured playback rate ${initialRate}, ` +
                `deferring initial seek to ASYNC_DONE: ${this._pendingInitialRateSeek}`);
