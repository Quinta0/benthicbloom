...
export class RotationManager {
    ...
    enable() {
        this._settingsSignals.push(
            ...
            this._settings.connect(`changed::${SettingsKey.WALLPAPER_RESCAN_REQUEST}`, () => this._reloadImages()),
    ...
    async _applyWallpaper(path) {
        ...
        if (this._settings.get_boolean(SettingsKey.TRANSITION_ENABLED) && previousPath)
            this._playTransition(previousPath);
    ...
    /**
     * The real background actor doesn't animate changes on its own, so we
     * paint the *old* image full-screen on a throwaway overlay right as the
     * new image is set underneath, then animate the overlay away to reveal
     * it. Which animation runs is picked per monitor by style.
     */
    _playTransition(previousPath) {
        this._clearTransitionOverlays();
